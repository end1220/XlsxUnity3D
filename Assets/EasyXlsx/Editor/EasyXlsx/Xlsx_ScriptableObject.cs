...
public class Xlsx_ScriptableObject
{
	...
	public static void xlsx_to_asset(string srcFilePath, string outputPath)
	{
		try
		{
			...
			_to_asset(fileName, outputPath, sheetData);
		}
	...
	public static void xlsx_to_cs(string srcFilePath, string outputPath)
	{
		try
		{
			int index = srcFilePath.LastIndexOf("/") + 1;
	...
	public static void _to_asset(string fileName, string outputPath, XlsxReader.SheetData sheetData)
	{
		try
		{
